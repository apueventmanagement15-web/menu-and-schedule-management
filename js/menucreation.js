...
function addallform(){
    ...
    if (anyvalidform) {
        showDialog("All forms have been added");
    }
}
...
function validateinput(formID) {
  ...
  for (let field of requiredFields) {
    ...
    if (!el || isEmpty(el.value)) {
      showDialog(`❗ Please fill in the "${field.label}" field properly.`);
      return false;
    }
  }

  // Check that at least one checkbox is selected in each group
  const drinks = getcheckboxvalues(`drinks${formID}[]`,`drinkother${form_count}`);
  const refreshment = getcheckboxvalues(`refreshment${formID}[]`,`refreshmentother${form_count}`);
  const food = getcheckboxvalues(`food${formID}[]`,`foodother${form_count}`);

  if (drinks.length === 0) {
    showDialog("❗ Please select at least one drink option.");
    return false;
  }
  if (refreshment.length === 0) {
    showDialog("❗ Please select at least one refreshment option.");
    return false;
  }
  if (food.length === 0) {
    showDialog("❗ Please select at least one food option.");
    return false;
  }
...
function resetform() {
  ...
  fieldsets.forEach(fieldset => {
    if (fieldset.id !== "form0" && fieldset.id !=="scheduledate") {
      // Delete all fieldsets except form0 and scheduledate
      fieldset.remove();
    } else {
